...
public class Bookshare_Periodical_Listing extends ListActivity {
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == DATA_FETCHED) {
				...
				new GetPeriodicalsTask().execute();
	...
	class GetPeriodicalsTask extends AsyncTask {
		...
		protected Object doInBackground(Object... arg0) {
			...
			// For each bean object stored in the vector, create a row in the
			// list
		...
		protected void onPostExecute(Object result) {
			...
			final MySimpleAdapter simpleadapter = new MySimpleAdapter(
					...
					new String[] { "title", "authors", "icon", "download_icon",
							"periodical_id" }, new int[] { R.id.text1,
							R.id.text2, R.id.row_icon,
			...
			if (null != decorView) {
				...
				if (vectorResults.isEmpty()) {
					resultsMessage = resources
							.getString(R.string.search_complete_no_books);
					setResult(InternalReturnCodes.NO_BOOKS_FOUND);
					confirmAndClose("no books found", 3000);
				} else {
					resultsMessage = resources
							.getString(R.string.search_complete_with_books);
				}
