...
public class Bookshare_Menu extends ListActivity {
	...
	private String URI_String = Bookshare_Webservice_Login.BOOKSHARE_API_PROTOCOL
			+ Bookshare_Webservice_Login.BOOKSHARE_API_HOST + "/book/";
	private String URI_Periodical_String = Bookshare_Webservice_Login.BOOKSHARE_API_PROTOCOL
			+ Bookshare_Webservice_Login.BOOKSHARE_API_HOST + "/periodical/";
	...
	private final int START_BOOKSHARE_PERIODICAL_LISTING_ACTIVITY = 3; // This
																		// is to
																		// start
																		// listing
																		// periodicals
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String logInMenuItem = isFree ? getResources().getString(
				R.string.bks_menu_log_in) : getResources().getString(
				R.string.bks_menu_log_out);
		// Create a TreeMap for use in the SimpleAdapter
		String[] items = {
				getResources().getString(R.string.bks_menu_title_label),
				getResources().getString(R.string.bks_menu_author_label),
				getResources().getString(R.string.bks_menu_isbn_label),
				getResources().getString(R.string.bks_menu_latest_label),
				getResources().getString(R.string.bks_menu_popular_label),
				getResources().getString(R.string.bks_menu_periodicals_label),
				logInMenuItem };
		...
		// Construct a SimpleAdapter which will serve as data source for this
		// ListView
		MySimpleAdapter simpleadapter = new MySimpleAdapter(this, list,
				R.layout.bookshare_menu_item, new String[] { "Name", "icon" },
		...
		dialog_search_term = (EditText) dialog
				.findViewById(R.id.bookshare_dialog_search_edit_txt);
		dialog_search_title = (TextView) dialog
				.findViewById(R.id.bookshare_dialog_search_txt);
		dialog_example_text = (TextView) dialog
				.findViewById(R.id.bookshare_dialog_search_example);
		dialog_ok = (Button) dialog.findViewById(R.id.bookshare_dialog_btn_ok);
		Button dialog_cancel = (Button) dialog
				.findViewById(R.id.bookshare_dialog_btn_cancel);
		...
		lv.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view,
				...
				TextView txt_name = (TextView) row_view
						.findViewById(R.id.text1);

				if (txt_name.getText()
						.equals(getResources().getString(
								R.string.bks_menu_title_label))) {
					showTitleSearch();
				} else if (txt_name.getText().equals(
						getResources()
								.getString(R.string.bks_menu_author_label))) {
					showAuthorSearch();
				} else if (txt_name.getText().equals(
						getResources().getString(R.string.bks_menu_isbn_label))) {
					showISBNSearch();
						...
						getResources()
								.getString(R.string.bks_menu_latest_label))) {
					// Changed the behavior to search for the latest book
					// without having to enter the date range
					...
					if (isFree)
						search_term = URI_String + "latest?api_key="
								+ developerKey;
					else
						search_term = URI_String + "latest/for/" + username
								+ "?api_key=" + developerKey;
					query_type = LATEST_REQUEST;
					intent = new Intent(getApplicationContext(),
							Bookshare_Books_Listing.class);
					...
					startActivityForResult(intent,
							START_BOOKSHARE_BOOKS_LISTING_ACTIVITY);
				}
				...
				// Option to search for popular books on Bookshare website
					...
					if (isFree)
						search_term = URI_String + "popular?api_key="
								+ developerKey;
					else
						search_term = URI_String + "popular/for/" + username
								+ "?api_key=" + developerKey;
				...
				// This is when user clickes on 'All Periodicals' (thushv)
					...
					if (!isFree)
						search_term = URI_Periodical_String + "list/for/"
								+ username + "?api_key=" + developerKey;
					else
						search_term = URI_Periodical_String + "list?api_key="
								+ developerKey;

					query_type = ALL_PERIODICAL_REQUEST;
					intent = new Intent(getApplicationContext(),
							Bookshare_Periodical_Listing.class);
					...
					if (!isFree) {
						...
						intent.putExtra("password", password);
						startActivityForResult(intent,
								START_BOOKSHARE_PERIODICAL_LISTING_ACTIVITY);
					} else {
						...
						getResources().getString(R.string.bks_menu_log_in))) {
					Intent intent = new Intent(getApplicationContext(),
							Bookshare_Webservice_Login.class);
					startActivity(intent);
					finish();
				}

				else if (txt_name.getText().equals(
						getResources().getString(R.string.bks_menu_log_out))) {
					final Dialog confirmDialog = new Dialog(myActivity);
					confirmDialog.setTitle(getResources().getString(
							R.string.accessible_alert_title));
					confirmDialog
							.setContentView(R.layout.accessible_alert_dialog);
					TextView confirmation = (TextView) confirmDialog
							.findViewById(R.id.bookshare_confirmation_message);
					confirmation.setText(getResources().getString(
							R.string.logout_dialog_message));
					Button yesButton = (Button) confirmDialog
							.findViewById(R.id.bookshare_dialog_btn_yes);
					Button noButton = (Button) confirmDialog
							.findViewById(R.id.bookshare_dialog_btn_no);

					yesButton.setOnClickListener(new OnClickListener() {
						@Override
						public void onClick(View v) {
							// Upon logout clear the stored login credentials
							SharedPreferences login = PreferenceManager
							...
							confirmAndClose(
									getResources()
											.getString(
													R.string.bks_menu_log_out_confirmation),
									2000);
						}
					});
	...
	private AlertDialog createLoginDialogBox() {
		...
		builder.setNegativeButton("Continue Anyway",
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						dialog.dismiss();
	...
	private void showAuthorSearch() {
		...
		showSearch(R.string.search_dialog_title_author,
				R.string.search_dialog_label_author,
				R.string.search_dialog_example_author,
				R.string.search_dialog_description_author,
				AUTHOR_SEARCH_REQUEST, InputType.TYPE_CLASS_TEXT);
	}
	...
	private void showTitleSearch() {
		...
		showSearch(R.string.search_dialog_title_title,
				R.string.search_dialog_label_title,
				R.string.search_dialog_example_title,
				R.string.search_dialog_description_title, TITLE_SEARCH_REQUEST,
				InputType.TYPE_CLASS_TEXT);
	}

	private void showISBNSearch() {
		intent = new Intent(getApplicationContext(),
				Bookshare_Book_Details.class);
		intent.putExtra(REQUEST_TYPE, ISBN_SEARCH_REQUEST);
		showSearch(R.string.search_dialog_title_isbn,
				R.string.search_dialog_label_isbn,
				R.string.search_dialog_example_isbn,
				R.string.search_dialog_description_isbn, ISBN_SEARCH_REQUEST,
				InputType.TYPE_CLASS_NUMBER);
	}

	private void showSearch(int titleId, int labelId, int exampleId,
			int contentDescriptionId, int queryType, int inputType) {
		...
		dialog_search_term.setContentDescription(getResources().getString(
	...
	protected void onStart() {
		...
		if (!isNetworkAvailable()) {
			confirmAndClose(
					getResources().getString(R.string.bks_menu_no_internet),
					3500);
		}
	}
	...
	private void doSearch() {
		...
		if (search_term.equals("")) {
			final VoiceableDialog finishedDialog = new VoiceableDialog(
					dialog_ok.getContext());
			String message = getResources().getString(
					R.string.empty_search_term_error);
		...
		if (query_type == TITLE_SEARCH_REQUEST) {
			...
			if (isFree)
				search_term = URI_String + "isbn/" + search_term + "?api_key="
						+ developerKey;
			else
				search_term = URI_String + "isbn/" + search_term + "/for/"
						+ username + "?api_key=" + developerKey;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == START_BOOKSHARE_BOOKS_LISTING_ACTIVITY) {
			if (resultCode == BOOKSHARE_BOOKS_LISTING_FINISHED) {
				...
				// can only get here from failed ISBN search (other search
