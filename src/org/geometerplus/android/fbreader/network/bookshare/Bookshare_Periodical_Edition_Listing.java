...
public class Bookshare_Periodical_Edition_Listing extends ListActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		requestType = intent.getIntExtra(Bookshare_Menu.REQUEST_TYPE,
				Bookshare_Menu.PERIODICAL_EDITION_REQUEST);
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == DATA_FETCHED) {
				...
				if (responseType == EDITION_LIST_RESPONSE) {
					...
					if (current_result_page > 1) {
						createPageChanger("Previous Page",
								PREVIOUS_PAGE_BOOK_ID,
			...
			MySimpleAdapter simpleadapter = new MySimpleAdapter(
					...
					R.layout.bookshare_menu_item, new String[] { "title",
							"edition", "download_icon", "book_id" }, new int[] {
							R.id.text1, R.id.text2,
							R.id.bookshare_download_icon, R.id.bookId });
			...
			lv.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> parent, View view,
					...
					TextView periodicalEdition = (TextView) row_view
					...
					for (Bookshare_Periodical_Edition_Bean bean : vectorResults) {
						...
						StringBuilder editionBuilder = new StringBuilder(bean
								.getEdition());
						editionBuilder.insert(2, "/");
						editionBuilder.insert(5, "/");
						editionBuilder.insert(0, "Edition: ");
						editionBuilder.append("\t Revision: "
								+ bean.getRevision());

						if (editionBuilder.toString().equalsIgnoreCase(
								periodicalEdition.getText().toString().trim())) {
							...
							if (isFree)
								uri = URI_BOOKSHARE_PERIODICAL_EDITION_SEARCH
										+ bookshare_ID + "/edition/"
										+ bookshare_edition + "/revision/"
										+ bookshare_revision + "?api_key="
										+ developerKey;
							else
								uri = URI_BOOKSHARE_PERIODICAL_EDITION_SEARCH
										+ bookshare_ID + "/edition/"
										+ bookshare_edition + "/revision/"
										+ bookshare_revision + "/for/"
							...
							if (!isFree) {
								...
								intent.putExtra("ID_SEARCH_URI", uri);
								intent.putExtra("PERIODICAL_TITLE",
										bookshare_title);
								intent.putExtra("PERIODICAL_ID", bookshare_ID);
								startActivityForResult(intent,
										START_BOOKSHARE_EDITION_DETAILS_ACTIVITY);

								// It might be not to give user suprises by
								// having a different view for periodicals
		...
		builder.setPositiveButton("Login",
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						intent.putExtra("periodical_revision",
								bookshare_revision);
						...
						intent.putExtra(Bookshare_Menu.REQUEST_TYPE,
								EDITION_METADATA_RESPONSE);
						dialog.dismiss();
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == START_BOOKSHARE_EDITION_DETAILS_ACTIVITY) {
			if (resultCode == BOOKSHARE_EDITION_DETAILS_FINISHED) {
				...
				finish();
			} else if (resultCode == InternalReturnCodes.NO_PERIODICALS_FOUND) {
	...
	private class MySimpleAdapter extends SimpleAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if ((data.get("edition") != null)
					&& ((String) data.get("edition")).length() > 0) {
